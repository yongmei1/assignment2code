...
int uart_init()
{
  ...
  for (i=0; i<4; i++){   // uart0 to uart2 are adjacent 
    ...
    *(up->base+0x38) |= 0x30;  // enable both TX 5 and RX 4
    //*(up->base+0x38) |= 0x10;  // enable both RX but not TX
...
// do we need this?
  ...
  kprintf("rx interrupt: mode=%x", sr&0x1F);
  if ((sr&0x1F)==0x1F)
    kprintf("%s", "SYS mode ");
  if ((sr&0x1F)==0x13)
    kprintf("%s", "SVC mode ");
...
int uputc(UART *up, char c)
{
  ...
  *(up->base+IMSC) |= 0x30; // 0011 0000: bit5=TX mask bit4=RX mask
   up->txon = 1;
}
